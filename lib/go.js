 ...
 * @param {object} ctx.env      - DB (D1) ve ASSETS binding'leri
 * @param {string} ctx.slug     - URL'deki slug
 * @param {(p: Promise<any>) => void} [ctx.waitUntil]
 */
export async function handleGo({ request, env, slug, waitUntil }) {
	let data;
	try {
		data = await loadLinks(env, request);
	} catch {
	...
	if (env?.DB && shouldCount(request)) {
		const task = recordClick(env.DB, link.slug).catch((error) => {
